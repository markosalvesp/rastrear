...
export async function buildReply(text, chatId, store) {
  ...
  if (cmd === "/seguir" || cmd === "/add") {
    ...
    return added
      ? `✅ Agora você segue <b>${esc(nick)}</b>.\nModo: 🛡️ AFK (avisa <b>conexão</b>, <b>morte</b>, <b>queda</b> e <b>drop</b>). Pra receber tudo: <code>/modo ${esc(nick)} tudo</code>.`
  ...
  if (cmd === "/lista" || cmd === "/list") {
    ...
    return "⭐ Seus bonecos:\n" + items.map((it) => {
      const status = it.onlineNow ? "🟢 online" : "🔴 offline";
      return `• <b>${esc(it.nick)}</b> — ${status} · ${describeEvents(it.events)}`;
    }).join("\n");
  }
  if (cmd === "/modo") {
  ...
  const bulkPvpMute =
    (cmd === "/mutar" && ["todos", "all"].includes(nick.toLowerCase()) && ["pvp", "kill", "kills"].includes(extra)) ||
    (cmd === "/mute" && ["kill", "kills", "pvp"].includes(nick.toLowerCase()) && ["todos", "all"].includes(extra));
  const bulkPvpRestore =
    (cmd === "/avisar" && ["todos", "all"].includes(nick.toLowerCase()) && ["pvp", "kill", "kills"].includes(extra)) ||
    (cmd === "/unmute" && ["kill", "kills", "pvp"].includes(nick.toLowerCase()) && ["todos", "all"].includes(extra));

  if (bulkPvpMute) {
    const result = await store.mutePvpAll(chatId);
    if (!result.total) return "Você ainda não segue nenhum boneco.";
    return `🔕 Mortes e abates desligados para <b>${result.total}</b> boneco${result.total === 1 ? "" : "s"}.\nDesfazer: <code>/avisar todos pvp</code>`;
  }
  if (bulkPvpRestore) {
    const result = await store.restorePvpAll(chatId);
    if (!result.restored) return "Não existe um mute global de PvP para desfazer.";
    return `🔔 Preferências anteriores restauradas para <b>${result.total}</b> boneco${result.total === 1 ? "" : "s"}.`;
  }
