...
import {
  follow, unfollow, listFor, setMode, toggleType, eventsFor, toggleBoss, touch,
...
const store = {
  ...
  listWithModes: (c) => listFor(c).map((n) => ({
    nick: n,
    events: eventsFor(c, n),
    onlineNow: getPresence(n).onlineNow,
  })),
  setMode: (c, n, p) => setMode(c, n, p),
  toggleType: (c, n, t, on) => toggleType(c, n, t, on),
  mutePvpAll: (c) => mutePvpAll(c),
  restorePvpAll: (c) => restorePvpAll(c),
  toggleBoss: (c) => toggleBoss(c),
  presence: (n) => getPresence(n),
};

async function handle(msg) {
  if (!msg.text) return;
  touch(msg.chat.id); // registra o chat (recebe avisos de boss por padrão)
