...
export const PRESETS = {
  ...
  afk: ["connect", "death", "disconnect", "drop"], // conexão + morte + queda + o que dropou
  pvp: ["kill", "death"],
  drops: ["drop"],
};

// Modo padrão ao seguir um boneco (AFK: conexão + morte + queda + drop).
export const DEFAULT_EVENTS = PRESETS.afk;
const LEGACY_AFK_EVENTS = ["death", "disconnect", "drop"];

// Preferências AFK salvas antes de conexão entrar no preset continuam sendo
// reconhecidas como AFK. Assim os inscritos atuais recebem o novo padrão sem
// precisar seguir novamente nem regravar os dados no Blobs.
export function resolveEvents(events) {
  if (!Array.isArray(events)) return DEFAULT_EVENTS;
  const legacyAfk =
    events.length === LEGACY_AFK_EVENTS.length &&
    LEGACY_AFK_EVENTS.every((event) => events.includes(event));
  return legacyAfk ? DEFAULT_EVENTS : events;
}
...
export function mutePvpForAll(chat) {
  ...
  for (const nick of nicks) {
    ...
    const current = explicit ? resolveEvents(chat.prefs[key]) : DEFAULT_EVENTS;
...
export const HELP =
  ...
  "• <code>/modo NICK afk</code> 🛡️ — conexão, morte, queda e drop (ideal AFK)\n" +
