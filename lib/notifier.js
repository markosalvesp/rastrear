...
async function apply(nick, type, diff) {
  ...
  for (const msg of diff.messages)
    for (const chatId of followersOf(nick)) if (!(await sendMessage(chatId, msg))) allOk = false;
  if (allOk && getMark(nick, type) !== diff.newMark) setMark(nick, type, diff.newMark);
}
