...
function store() {
  // O próximo tick precisa enxergar imediatamente o estado salvo pelo anterior;
  // caso contrário a mesma transição pode ser detectada e enviada duas vezes.
  return getStore({ name: "presence", consistency: "strong" });
}
...
export async function pollOnce() {
  ...
  const connects = []; // nicks que conectaram agora (hora exata conhecida)
  const disconnects = []; // { nick, durationSec }
  ...
  for (const nick of online) {
    ...
    if (!p.online) {
      ...
      if (exact) { p.lastConnect = now; connects.push(nick); }
  ...
  for (const [nick, p] of Object.entries(state.players)) {
    if (p.online && !online.has(nick)) {
      ...
      disconnects.push({ nick, durationSec: s?.durationSec });
    }
  }
  ...
  return { liveCount: online.size, connects, disconnects };
}
