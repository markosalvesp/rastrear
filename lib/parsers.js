...
export function parseDuelLog($input) {
  ...
  $(".dest-log-combat-card").each((_, el) => {
    ...
    // Convenção do Destiny Tale (Priston Tale): quem MATA vira caótico e fica com o
    // NOME VERMELHO (o site marca esse lado como "--right/--loser"). Quem MORRE fica
    // com o nome DOURADO ("--left/--winner"). Ou seja: vermelho = matou, dourado = morreu.
    const killer = side(".dest-log-combat-combatant--right"); // vermelho = matou
    const victim = side(".dest-log-combat-combatant--left"); //  dourado  = morreu
    if (!killer.nick && !victim.nick) return;
    rows.push({
      winner: killer.nick, // "winner" = quem matou
      winnerClass: killer.class,
      loser: victim.nick, //  "loser" = quem morreu
      loserClass: victim.class,
      time:
