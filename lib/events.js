...
const SEEN_EMPTY = "∅"; // "já vi vazio": segue zerado, mas o próximo evento é novo

export function diffLog(rows, keyOf, msgOf, mark) {
  // Sem registros: se ainda não tinha marcador, grava "visto vazio" — assim o
  // PRÓXIMO evento (1º drop/luta de um boneco zerado) conta como novo, não histórico.
  if (!rows || !rows.length) return { messages: [], newMark: mark == null ? SEEN_EMPTY : mark };
  const newestKey = keyOf(rows[0]);
  // 1ª vez com registros existentes: baseline sem avisar (não despeja histórico).
  // Mas se já tínhamos "visto vazio", tudo que aparecer agora é novo -> avisa.
...
// Geradores de texto (usados pelo motor global, cruzando o feed com os seguidos).
export const dropText = (r) =>
  `🎁 <b>${esc(r.nick)}</b> dropou <b>${esc(r.item)}</b>${r.map ? ` <i>(${esc(r.map)})</i>` : ""} — ${esc(r.time || "")}`;
export const killText = (r) =>
  `⚔️ <b>${esc(r.winner)}</b> matou <b>${esc(r.loser)}</b> (${esc(r.loserClass || "?")}) — ${esc(r.time || "")}`;
export const deathText = (r) =>
  `💀 <b>${esc(r.loser)}</b> morreu para <b>${esc(r.winner)}</b> (${esc(r.winnerClass || "?")}) — ${esc(r.time || "")}`;
