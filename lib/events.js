...
export function dropsDiff(nick, rows, mark) {
  return diffLog(rows, (r) => `${r.time}|${r.item}`, (r) =>
    `🎁 <b>${esc(nick)}</b> dropou <b>${esc(r.item)}</b>${r.map ? ` <i>(${esc(r.map)})</i>` : ""} — ${esc(r.time || "")}`, mark);
}
...
export function pvpDiff(nick, rows, mark) {
  ...
  return diffLog(rows, (r) => `${r.time}|${r.winner}|${r.loser}`, (r) =>
    String(r.loser || "").toLowerCase() === n
      ? `💀 <b>${esc(nick)}</b> morreu para <b>${esc(r.winner)}</b> (${esc(r.winnerClass || "?")}) — ${esc(r.time || "")}`
      : `⚔️ <b>${esc(nick)}</b> matou <b>${esc(r.loser)}</b> (${esc(r.loserClass || "?")}) — ${esc(r.time || "")}`, mark);
}
