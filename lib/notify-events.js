...
const BUDGET_MS = 6000; // orçamento de tempo (função tem ~10s; o pollOnce já usou ~2s antes)
...
export async function runEventChecks() {
  ...
  while (done < nicks.length && done < BATCH && Date.now() - started < BUDGET_MS) {
    ...
    marks.rot = idx;
    // salva a cada boneco: se a função for cortada no meio (timeout), o que já
    // foi processado NÃO se perde -> não reenvia (era a causa dos duplicados).
