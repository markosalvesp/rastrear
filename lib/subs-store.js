...
export async function loadChats() {
  ...
  try {
    ...
    let cursor;
    do {
      const res = await store.list({ prefix: "chat:", cursor });
...
export async function updateChat(chatId, mutator) {
  ...
  let c;
  try { c = await store.get(key, { type: "json" }); } catch { c = null; }
  c = c || { nicks: [] };
  const result = mutator(c);
  try { await store.setJSON(key, c); } catch (e) { console.error("updateChat:", e.message); }
  return result;
}
export async function deleteChat(chatId) {
  try { await subs().delete(CKEY(chatId)); } catch {}
}
