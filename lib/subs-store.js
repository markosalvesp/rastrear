...
// Escrita ATÔMICA: lê com etag, aplica a mudança, grava só se o etag não mudou.
// Se dois escreverem juntos, um refaz sobre a versão mais nova (ninguém é apagado).
// Se a LEITURA falhar, NÃO grava (evita sobrescrever a lista com vazio).
export async function updateChats(mutator) {
  const store = getStore("subs");
  for (let attempt = 0; attempt < 6; attempt++) {
    let data, etag;
    try {
      const e = await store.getWithMetadata("chats", { type: "json" });
      data = (e && e.data) || { chats: {} };
      etag = e && e.etag;
    } catch {
      return { ok: false, result: undefined }; // leitura falhou -> aborta (não apaga)
    }
    if (!data.chats) data.chats = {};
    const result = mutator(data);
    try {
      const opts = etag ? { onlyIfMatch: etag } : { onlyIfNew: true };
      const res = await store.set("chats", JSON.stringify(data), opts);
      if (res && res.modified === false) continue; // outra escrita ganhou -> refaz
      return { ok: true, result };
    } catch {
      continue; // provável conflito de etag -> refaz
    }
  }
...
export function eventsFor(s, chatId, nick) {
  return s.chats[chatId]?.prefs?.[norm(nick)] || DEFAULT_EVENTS;
}
...
export function toggleBoss(s, chatId) {
  const c = (s.chats[chatId] ||= { nicks: [] });
  c.boss = c.boss === false; // se estava ligado -> desliga; se desligado -> liga
  return c.boss;
}
export const bossChats = (s) =>
  Object.entries(s.chats).filter(([, c]) => c.boss !== false).map(([id]) => id);

// Garante que o chat existe (pra receber avisos de boss por padrão).
export function touchChat(s, chatId) {
  if (!s.chats[chatId]) { s.chats[chatId] = { nicks: [] }; return true; }
  return false;
}
