...
export function toggleType(c, nick, type, on) {
  ...
  const cur = new Set(resolveEvents(c.prefs?.[norm(nick)]));
...
export const mutePvpAll = (c) => mutePvpForAll(c);
export const restorePvpAll = (c) => restorePvpForAll(c);

// ---- helpers de LEITURA (operam no { chats } do loadChats) ----
export const listFor = (s, chatId) => s.chats[chatId]?.nicks || [];
export const eventsFor = (s, chatId, nick) => resolveEvents(s.chats[chatId]?.prefs?.[norm(nick)]);
