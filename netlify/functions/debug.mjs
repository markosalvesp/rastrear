// Diagnóstico temporário. ?k=<TELEGRAM_BOT_TOKEN>. Remover depois.
...
export default async (req) => {
  ...
  const meta = await getStore("meta").get("lastTick", { type: "json" }).catch(() => null);
  return Response.json({ chats, marks, lastTick: meta, now: Date.now() });
};
